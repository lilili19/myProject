<template>
	<ul class="menu">
        <li v-for="(item,index) in menuList" :key="index">
            <div class="title" @click="$emit('changeHidden', index)" @contextmenu='(e) => userName === item.userName ? groupDetail(e, index) : ""'>
                <span>
                    {{item.title}}
                    <span style="color: #00ff0a">{{userName === item.userName ? '' : ` (${item.userName})`}}</span>
                </span>
            ...
            <ul :style="{ height: item.hidden ? 0 : (42 * item.children.length + 'px')}">
                <li 
                    ...
                    :key="index2" 
                    class="title2" 
                    @contextmenu='(e) => userName === item.userName ? groupDetail(e, index, index2) :""'
