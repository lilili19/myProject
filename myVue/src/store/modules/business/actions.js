...
export default {
    ...
    async menuDel({commit}, {data, callBack}) { // 删除分组或者增加项目请求
        const url = '/menu/del';
        await http.post({url, data}, {
            success: (data) => {
                callBack && callBack(data);
    ...
    async projectDel({commit}, {data, callBack}) { // 删除项目接口
    ...
    async updateState({commit}, {data, callBack}) { // 代理请求
        const url = '/user/updateState';
        await http.post({url, data}, {
            success: (data) => {
                callBack && callBack(data);
            }
