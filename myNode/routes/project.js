...
// 检验非法操作（操作别人的项目）
const ischeakUserName = async (userName, uuid, mySql, res) => {
    ...
    if(queryData.length !== 1) { // 不存在的uuid 或者 多条 uuid（没传uuid）
        mySql.end();
        const resObj = {
            data: null,
            msg: queryData.length === 0 ? '缺少uuid' : '请勿非法操作',
            code:  400
        }
        return res.send(JSON.stringify(resObj));
    }
    if(userName !== queryData[0].userName) {
        mySql.end();
        const resObj = {
            data: null,
            msg: '请勿非法操作',
...
router.post('/addEdit', async (req, res) => {
    ...
    */
    ...
    if(await ischeakUserName(userName, obj.uuid, mySql, res)) return;
...
router.post('/del', async (req, res) => {
    ...
    */
    ...
    if(await ischeakUserName(userName, uuid, mySql, res)) return;
